...
public class ListResourcesQueryHandler
    : ResourceRequestHandlerBase, IQueryHandler<ListResourcesQuery, ICollection>
{
    ...
    /// <inheritdoc/>
    public virtual async Task<ApiResponse<ICollection>> Handle(ListResourcesQuery query, CancellationToken cancellationToken)
    {
        return this.Ok(await this.Repository.ListAsync(query.Group, query.Version, query.Plural, query.Namespace, query.LabelSelectors, query.MaxResults, query.ContinuationToken, cancellationToken).ConfigureAwait(false));
    }
